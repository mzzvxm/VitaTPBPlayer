...
int tpb_search(const char *query, TpbResult *results, int max_results, char *error_out, size_t error_out_size) {
    ...
    curl = curl_easy_init();
    if (!curl) {
        snprintf(error_out, error_out_size, "curl_easy_init() falhou.");
        free(chunk.memory);
        return -1;
    }
    ...
    if (res != CURLE_OK) {
        snprintf(error_out, error_out_size, "Falha de rede: %s", curl_easy_strerror(res));
    ...
    if (chunk.size == 0) {
        snprintf(error_out, error_out_size, "A API retornou uma resposta vazia.");
        curl_easy_cleanup(curl);
        free(chunk.memory);
        return -1;
    }
    ...
    while (count < max_results && (current_pos = strstr(current_pos, "\"id\":"))) {
        ...
        json_extract_string(current_pos, "size", size_str, sizeof(size_str));
