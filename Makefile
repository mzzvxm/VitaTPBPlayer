...
# Lista de bibliotecas CORRIGIDA, com a adição de -lbz2
...
$(TARGET).vpk: eboot.bin param.sfo
	vita-pack-vpk -s param.sfo -b eboot.bin --add resources/font.ttf=font.ttf $@
...
clean:
	rm -f $(TARGET).vpk $(TARGET).velf $(TARGET).elf $(OBJS) eboot.bin param.sfo
